...
do
local Executive1 = 247209396 --put your id here(BOT OWNER ID)
...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('channel#id'..msg.to.id, 'set Rank for ('..name..') To : '..value, ok_cb,  true)
  end
end
local function remrank(msg, name, value) -- setrank function
  local hash = nil
  if msg.to.peer_type == 'channel' then
    hash = 'rank:'..msg.to.id..':variables'
  end
  if hash then
	...
	return send_msg('channel#id'..msg.to.id, 'remove Rank for ('..name..') ', ok_cb,  true)
  end
end
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   text = text..'4-Rank : '..value..'\n'
		 ...
		 if result.from.peer_id == tonumber(Executive1) then
		       text = text..'5-Sudo : Yes \n'
		     elseif result.from.peer_id == tonumber(Executive2) then
		       text = text..'5-Sudo : Yes \n'
		     elseif result.from.peer_id == tonumber(Executive3) then
		       text = text..'5-Sudo : Yes \n'
		     elseif result.from.peer_id == tonumber(Executive4) then
		       text = text..'5-Sudo : Yes \n'
		     elseif result.from.peer_id == tonumber(Executive5) then
		       text = text..'5-Sudo : Yes \n'
			 else
		       text = text..'5-Sudo : No \n'
		 end
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = '1-Full name : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
               ..'2-Username : '..Username..'\n'
               ..'3-ID : '..result.from.peer_id..'\n'
	local hash = 'rank:'..result.to.peer_id..':variables'
...
setrank(result, result.from.peer_id, value)
end
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'remrank' then
  local hash = 'usecommands:'..msg.from.id..':'..msg.to.id
  redis:incr(hash)
  if not is_momod(msg) then
    return "For moderators only!"
  ...
  local name = string.sub(matches[2], 1, 50)
  local value = string.sub(matches[3], 1, 1000)
  local text = remrank(msg, name, value)
  ...
  if msg.from.username then
   ...
   local hash = 'rank:'..msg.to.id..':variables'
    ...
    if msg.to.type == 'channel' then
	 text = text..'Supergroup name : '..msg.to.title..'\n'
     text = text..'Supergroup ID : '..msg.to.id
    end
	text = text..'\nGPDragon'
